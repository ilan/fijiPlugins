...
public class PetCtPanel extends JPanel implements MouseListener, MouseMotionListener,
	...
	long nanoTime;
	String petSeriesName = null, operatorName = null;
	...
	boolean isGated = false, runGated = false;
	...
	double syncAxial, syncCoronal, syncSagital;
	...
	void incrSlicePosition( int diff, int mousePage, boolean spinMIP, double zDiffCm) {
		...
		if( diff == 1000) {	// home key
			switch( m_sliceType) {
				...
				case JFijiPipe.DSP_CORONAL:
					petCoronal = (petPipe.data1.width - 1) / 2;
					break;
			...
			case JFijiPipe.DSP_AXIAL:
				...
				if( mousePage == 0) { // notify from another study
					petAxial -= zDiffCm / petPipe.avgSliceDiff;
					break;
				}
				zDiff = diff*factor;
				petAxial -= zDiff;
				zDiff *= petPipe.avgSliceDiff;
				break;

			case JFijiPipe.DSP_CORONAL:
				if( mousePage == 0) { // notify from another study
					petCoronal += zDiffCm / petPipe.getPixelSpacing(1);
					break;
				}
				zDiff = diff*invert;
				petCoronal += zDiff;
				zDiff *= petPipe.getPixelSpacing(1);
				break;

			case JFijiPipe.DSP_SAGITAL:
				if( mousePage == 0) { // notify from another study
					petSagital += zDiffCm / petPipe.getPixelSpacing(0);
					break;
				}
				zDiff = diff*invert;
				petSagital += zDiff;
				zDiff *= petPipe.getPixelSpacing(0);
				break;
		}
//		IJ.log("axial =" + petAxial + ", coronal = " + petCoronal + ", sagital = " + petSagital + ", diff =" + diff);
	...
	protected void maybeIncrSlicePosition(int diff, int mousePage, double zDiffCm) {
		...
		if( mouse1.page == 3 && showMip && !runCine) {}
		else {
...
//		IJ.log("About to background load.");
	...
	void updateDisp3Value(boolean corCT) {
		...
		dstPipe.mriOffSag = cpyPipe.mriOffSag;
		d3frm.repaint();
	}
	...
	void doActualLoadData() {
		...
		ImageJ ij;
		String tmp1;
		double aspect, height1, halfPixel, xshift=0, yshift=0, tmpd, ptY=0;
		Double ctY;
...
//		IJ.log("Done load");
		...
		petPipe.mriOffY0 = ChoosePetCt.round((1.0 - aspect)*petPipe.data1.width/2);
...
//					IJ.log("panZMri2Pet = " + ctY.toString());
		...
		petCoronal = petSagital = (petPipe.data1.width-1) / 2;
		...
		if( petRelativeZoom >= 1.0) {
			...
			halfPixel = 0.001 + petPipe.getPixelSpacing(0) / 2;
			...
			xshift = fact1*xshift/petPipe.getPixelSpacing(0);
			yshift = fact1*yshift/petPipe.getPixelSpacing(1);
			...
			halfPixel = 0.001 + ctPipe.getPixelSpacing(0) / 2;
			if(Math.abs(xshift) < halfPixel) xshift = 0;
			if(Math.abs(yshift) < halfPixel) yshift = 0;
			if(Math.abs(xshift) > 60*halfPixel) xshift = 0;	// Meckel S shows problem
			if(Math.abs(yshift) > 60*halfPixel) yshift = 0;
			xshift = -xshift/ctPipe.getPixelSpacing(0);
			yshift = -yshift/ctPipe.getPixelSpacing(1);
		...
		for( i=0; i<n; i++) {
			...
			if( currPipe != null) {
				...
				height1 = currPipe.data1.height * currPipe.data1.y2XMri;
				aspect = 1.0 * height1 / currPipe.data1.width;
				currPipe.aspect = aspect;
				currPipe.mriOffY0 = ChoosePetCt.round((1.0 - aspect)*currPipe.data1.width/2);
				currPipe.corFactor = (petPipe.zoomX * currPipe.data1.height) / (petPipe.data1.height * currPipe.zoomX * aspect);
				...
				currPipe.corOffset = (currPipe.sagOffset - currPipe.mriOffY0)/currPipe.data1.y2XMri;
...
//		parent.setTitle(parent.getTitleBar());
	...
	void maybeAddBlankSlicesInPet() {
		...
		if( petPipe.data1.spacingBetweenSlices != upetPipe.data1.spacingBetweenSlices) return;
	...
	void layoutMip(Graphics2D g, boolean fused) {
		...
		if( pet1 != null) {
			...
			if( m_sliceType ==JFijiPipe.DSP_CORONAL) {
				pet1.prepareCoronalSagital(petCoronal, -1, petColor, gateOffset);
				pet1.drawCorSagImages(g, scl1, this, true);	// coronal
			}
			if( m_sliceType == JFijiPipe.DSP_SAGITAL) {
				pet1.prepareCoronalSagital(-1, petSagital, petColor, gateOffset);
				pet1.drawCorSagImages(g, scl1, this, false);	// sagital
			}
			if( m_sliceType == JFijiPipe.DSP_OBLIQUE) {
				if(m_oblique != null) m_oblique.draw(g, scl1, pet1, petColor, petAxial + upetOffset);
			}
		} else printMissing(g);
		...
		if( ct1 != null) {
			...
			if( ctPos >= 0) {
				...
				if( m_sliceType == JFijiPipe.DSP_CORONAL) {
					ct1.prepareCoronalSagital(petCoronal, -1, ctMriColor, 0);
					ct1.drawCorSagImages(g, scl2, this, true);	// coronal
				}
				if( m_sliceType == JFijiPipe.DSP_SAGITAL) {
					ct1.prepareCoronalSagital(-1, petSagital, ctMriColor, 0);
...
//		scale1 = scale0;
	...
	void setCineIndx( int indx) {
		...
		if( reproPipe != null) reproPipe.cineIndx = indx;
	}
	...
	void checkLimits() {
		...
		if( petCoronal >= petPipe.data1.width - 1)
	...
	protected double setPetRelativeZoom(JFijiPipe petMri) {
		...
		z1 = petMri.data1.pixelSpacing[0] * origWidth / petMri.data1.y2XMri;
...
//	If it is positive this the Peleg case and it needs more care.
	...
	int getView3Slop() {
		double slop = Math.abs((ctPipe.getZpos(ctPipe.data1.numFrms-1) - ctPipe.getZpos(0)) / ctPipe.data1.spacingBetweenSlices);
...
/*			petAxial = syncAxial;
	...
	void notificationOfXYshift(int slType, Point2d diff) {
		...
		if( diff != null) {
			...
			diffSag = diff.x/petPipe.getPixelSpacing(0);
			switch( slType) {
				case JFijiPipe.DSP_AXIAL:
					petSagital += diffSag;
					petCoronal += diff.y/petPipe.getPixelSpacing(1);
					break;
				...
				case JFijiPipe.DSP_SAGITAL:
					petCoronal += diff.x/petPipe.getPixelSpacing(1);
					petAxial += diffAx;
					break;
			}
			parent.update3Display();
		...
		diffSag = (petSagital - syncSagital)*petPipe.getPixelSpacing(0);
		diffCor = (petCoronal - syncCoronal)*petPipe.getPixelSpacing(1);
		switch( slType) {
			case JFijiPipe.DSP_AXIAL:
				pt1.x = diffSag;
				pt1.y = diffCor;
...
/*		if( pt1.y < 0 || pt1.y >= width1) return;
		...
		totalSum = curMax = 0;
		numPts = z0 = z0a = 0;
		spacOut = Math.abs(pet1.data1.spacingBetweenSlices/ spac1);
		while(true) {
			spacz = 1.0;
...
//			currLine = pet1.data1.getLineOfData(angle, liney, z1+z0a);
	...
	double shift2Ct1( JFijiPipe currPipe, int xyChoice) {
		double inVal = petSagital, mriOff = currPipe.mriOffX;
	...
	private double calcPoints(double inVal, int x, int y, int z, int rn) {
		...
		if( retVal > curMax / 8) {
			suvPnt.addRearrangePoint(retVal, 0, x, y, z, rn);
		}
		return retVal;
	}

	int getSUVradius(boolean useZoom) {
		double spacing2 = 2*petPipe.data1.pixelSpacing[0];
	...
	void draw3CursorsAndSUV(Graphics2D g, boolean fused) {
		if(curPosition[0] == null) return;	// don't waste time
		int i, offY;
	...
	void draw3FrameText(Graphics2D g, JFijiPipe pet1) {
		...
		switch( m_sliceType) {
			...
			case JFijiPipe.DSP_CORONAL:
				...
				slice = ChoosePetCt.round(petCoronal) + 1 + "/" + petPipe.data1.width;
				break;
