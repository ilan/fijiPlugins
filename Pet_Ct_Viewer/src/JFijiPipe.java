...
public class JFijiPipe {
	...
	static final int DSP_OBLIQUE = 5;
	static final int PERCENT_AXIS = 10;
	static final int OFFX = 1;
	static final int OFFY = 2;
	static final int OFFZ = 3;
	static final int OFFZ0 = 4;
	static final int OFFTBLX = 5;	// these 2 entries are mutable
	static final int OFFTBLY = 6;
...
//			if( out1.y >= origHeight) out1.y = origHeight - 1;
	...
	protected Point pos2Scrn( Point2d pos1, double scale, int type, boolean intMode) {
		...
		double scalY = scale * data1.y2XMri;
		if( sliceType == DSP_SAGITAL) scalX = scalY;
		double offY = multYOff * origWidth * scalX;	// this was scalY, check it
	...
	protected Point pos2ScrnCnvt( Point2d pos1, double scale, int type) {
		...
		double scalY = scale * data1.y2XMri;
		if( sliceType == DSP_SAGITAL) scalX = scalY;
		double offY = multYOff * origWidth * scalX;	// this too was scalY, not typo
		double offY0 = mriOffY0 * scale;
		if( sliceType != DSP_AXIAL) offY0 = 0;
		if( useShiftXY) {
			xshft = shiftXY[0];
	...
	int getCTcursorShift( double inShift, double scale) {
		if( Math.abs(inShift) < 0.01) return 0;
		double scalY = scale * data1.y2XMri;
		if( zoom1*zoomX*data1.y2xFactor != 1.0) scalY *= getZoom(0);
		return ChoosePetCt.round(inShift*scalY);
	}
...
//		if( cnvt0 < 1) diffY *= cnvt0;
	...
	// incr = -1000 means reset zoom to 1.0
...
//			hsclw += scale*mriOffY0;
		...
		if( cm2 != null) {
			offscr = jDraw.createImage(w1,h1s);
		...
		try {
			...
			IJ.log("Error: attempt to draw from a null source");
		}
	}
	...
	void setAvgSliceDiff() {
		...
		if( diff == 0) diff = data1.sliceThickness;
		avgSliceDiff = diff;
	}

	void maybeSetCoronalDisplay() {
		// Gas,lo Jan has this problem
		int type = 0;
		String tmp1;
		double tmpThickness = data1.sliceThickness;
		double ratio = Math.abs(avgSliceDiff/tmpThickness);
		if( ratio < 0.55) type = 1;
		if( data1.zratio < 0.5) type = 2;	// more serious, fix it
		if( type == 0) return;
		// look at the slice spacing. It doesn't always exist.
		// when it does, it can be worse than slice thichness - but try...
		tmp1 = ChoosePetCt.getDicomValue(data1.metaData, "0018,0088");
		if( tmp1 != null) {
			double sliceSpacing = Math.abs(ChoosePetCt.parseDouble(tmp1));
			double absAvg = Math.abs(avgSliceDiff);
			if( sliceSpacing >= absAvg && sliceSpacing < Math.abs(tmpThickness)) {
				double tmp0 = sliceSpacing;
				if( tmpThickness < 0) tmp0 = -tmp0;
				data1.sliceThickness = tmp0;	// better value
				if( type == 1) {	// recalculate
					ratio = Math.abs(avgSliceDiff/tmp0);
					if( ratio > 0.6) return;
				}
			}
		}
		tmp1 = "If the fused coronal/sagital images look wrong, please send a copy of this data to ilan.tal@gmail.com.";
		if( type == 1) tmp1 = "Average slice thickness / Dicom slice thickness = " + PetCtFrame.roundN(ratio, 2) + " (maybe fusion incorrect?)";
		IJ.log(tmp1);
		if( type < 2) return;
		int i, j, n = getNormalizedNumFrms();
		double prevZ, thisZ, wantZ;
		CoronalDisplay = new int[n];
		thisZ = wantZ = prevZ = data1.zpos.get(0);
		CoronalDisplay[0] = 0;
		for( i=j=1; i<n; i++) {
			wantZ += avgSliceDiff;
			while( j < n-1) {
				thisZ = data1.zpos.get(j);
				if( avgSliceDiff > 0) {
					if( wantZ <= thisZ) break;
				} else {
					if( wantZ >= thisZ) break;
				}
				prevZ = thisZ;
			...
			if( Math.abs(wantZ-prevZ) <= Math.abs(wantZ-thisZ)) CoronalDisplay[i] = j-1;
			else CoronalDisplay[i] = j;
		}
	}
...
//		if( yOff < 0) yOff = 0;
		w1 = w2 = ChoosePetCt.round(scale * w0);
...
//			if(isConverted()) w2 *= aspect;	// don't double correct
		...
		xOff = ChoosePetCt.round(scale*(tmpx+xSag)*mriScl);
//		xOff = ChoosePetCt.round(scale*(tmpx+xSag)*mriScl*zoom1);
//		xOff = 0;
		if( cm2 != null) {
			offscrH1 = h1;
			offscrY1 = yOff;
	...
	protected void drawFusedImage(Graphics2D g, double scale, JFijiPipe other1, Point pos1, JPanel jDraw) {
		...
		boolean useD1 = true;
		double d1=0, tmp = other1.getH2w()/getH2w();
		double fact0 = 1.0*w/other1.data1.width;
		double mriOffy0_f = other1.mriOffY0 * fact0;
		double gap1 = other1.mri1.getOff(OFFY,DSP_AXIAL)*other1.mriScl;

		if( isMIP()) {
			if( mriOffy0_f >= 0) 
			{
//				d1 = zoom1*mri1.edge1*gap1*fact0 + mriOffY0;
				d1 = (zoom1 - mri1.zm1)*gap1*fact0 + mriOffY0;
//				IJ.log("fused, zoom="+zoom1+", zm1="+mri1.zm1);
			} //else mriOffy0_f = 0;
			useD1 = false;
		}
/*		if( saveParent instanceof PetCtPanel) {
			boolean isMIP = ((PetCtPanel)saveParent).isMIPdisplay();
			if( mriOffy0 >= 0) {
				if( !isMIP) {
					mriOffy0 *= fact0;
					useD1 = true;
		...
		if( saveParent instanceof Display3Panel) {
			mriOffy0 = 0;
			useD1 = false;
//			mriOffy0 *= fact0;
		}*/
		// the offscreen already has zoom=1, so we only need the difference > 1
		if( useD1)
			d1 = (zoom1 - 1) * gap1*fact0;
		yOff1 = ChoosePetCt.round(scale*((multYOff*w / tmp) + mriOffy0_f + d1));
...
//		factor = (float) ((1000 - fuseLevel) * fuseFactor / 100000);
	...
	// determine if to use YOff or XOff. Not used.
	boolean isYOff() {
		return (Math.abs(mriOffY) >= Math.abs(mriOffX));
	}

	double getZpos( double petPos) {
		int ipetPos = ChoosePetCt.round(petPos);
		double mriOffZ = mri1.getOffSub(OFFZ, DSP_AXIAL, 0);
	...
	double prepareCoronalSagitalSub( double corSlice, double sagSlice, int colorMod1, int gateOffset, int type2, double petSag) {
		UpdateCoronalSagital( corSlice, sagSlice, gateOffset, type2);
		return makeCorDspImage(colorMod1, false, petSag);
	}

	double prepareCoronalSagital( double corSlice, double sagSlice, int colorMod1, int gateOffset, double petSag) {
		return prepareCoronalSagitalSub(corSlice, sagSlice, colorMod1, gateOffset, JFijiPipe.DSP_AXIAL, petSag);
	}
	
	void prepareAxialOblique( double axSlice, int secondaryType, int colorMod1) {
		UpdateAxialOblique(axSlice, secondaryType);
		startFrm = ChoosePetCt.round(axSlice);
		makeDspImage(colorMod1, 6);
	}
...
/*			if ( oldLevel != winLevel || oldWidth != winWidth) {
		...
		for (int j = 0; j < numDisp; j++) {
			fillSource(j+indx, type, 0, 0);
		}
	}

	double makeCorDspImage(int colorMod1, boolean axialDisplay, double petSag) {
		double sagShift = 0;
		...
		if( coronalSlice >= 0) fillSource(0, type2, 1, 0);
		if( sagitalSlice >= 0) sagShift = fillSource(1, type2, 2, petSag);
		return sagShift;
	}
	...
	// type1 = 5 for oblique display with coronal,sagital display
	// type1 = 6 for oblique display with primary axial display
	private double fillSource(int indx1, int type1, int offInx, double petSag) {
		if( indx1 >= data1.numFrms) return 0;
		MemoryImageSource[] currSource	= source;
		if( type1 == 2 || type1 == 5) currSource = corSrc;
		if( currSource == null) return 0;
		if( currSource[indx1] != null && cm2==null && type1 != 6) return 0;	// already calculated - fix
		...
		double scale, slope, slope0, slopeMax, xpan, ypan, xshft=0, yshft=0;
		double sagMRI = 1.0, sagShift = 0;
		short currShort, maxShort=0;
		int min1, sagCut0 = 0;
		boolean badY, goodLungY;
		int coef0 = data1.getCoefficentAll();
		Point pan1 = new Point();
		slope0 = data1.getRescaleSlope(indx1);
		slopeMax =  data1.getMaxRescaleSlope();
		if( isLog) {
			i = data1.maxPixel;
			logOff = i/32;
			...
			logFactor = slope0/slopeMax;
			if( type1 == 2 || type1 == 5 || type1 == 6) logFactor = 1.0;
			if( LogConv == null) LogConv = new short[32768];
		}
		slope = winSlope * slope0;
/*		stop = new Date();
		Double diff = (double)(stop.getTime() - start.getTime());
		diff /= 1000.;
		IJ.log("fill source " + diff.toString());*/
		origWidth = w1 = w1a = getDispWidthHeight(0);
		origHeight = getDispWidthHeight(1);
		if( type1 == 6) {	// raw pixels are not expanded
			origWidth = w1 = w1a = data1.width;
			origHeight = data1.height;
			corSrc = new MemoryImageSource[2];
			indx2 = 0;
			coef0 = 0;
			slope = winSlope * slopeMax;
		}
		zfact = getDispWidthHeight(3);	// 2 for tricubic
		xpan = pan.x;
		ypan = pan.y;
		...
		if( type1 == 2) {
			...
			if( indx1 > 0) { // either 0 or 1
				...
				xshft = yshft;
				xshft -= 1.5*mri1.getOff(OFFZ0, DSP_CORONAL );
				xshft += w1*(aspect-1.0)/2;
			} else {
				xshft -= 1.5*mri1.getOff(OFFZ0, DSP_AXIAL);	// x coronal shift
			}
//			xshft += mri1.getOff(OFFX, insertType);
			yshft = 0;
		...
		if(w1 != w1a) {
			...
			if(insertType == DSP_SAGITAL)
				xshft += (sizeX1-w1)*(aspect-1.0)/2;
			sagShift = -mriOffY0/(2*zoomX*zoom1*sagMRI);
			if(sagShift > 0) xshft += sagShift;	// don't add for Pensylvania coronal data
...
//		xshft += (w1a - sizeX1)/(2 * sagMRI);	// with zoom
	...
	//	if( sagShift != 0) IJ.log(sagShift);
		...
		switch( data1.depth) {
			case 32:
				float[] inFlt;
				double currFlt, fltFactor = data1.fltDataFactor;
				inFlt = data1.pixFloat.get(indx1);
				if( type1 == 2 || type1 == 5 || type1 == 6) inFlt = rawFloatPix.get(indx2);
				for( j=k=0; j<sizeY; j++) {
					badY = false;
					...
					for( i=sagCut0; i<sizeX; i++) {
						...
						if( badY || i1 < 0 || i1 >= origWidth) curr1 = 0;
							...
							currFlt = inFlt[off1+i1];
							if( isLog && currFlt > 0) {
								currShort = (short)(currFlt / fltFactor);
								if( currShort > maxShort)
									maxShort = currShort;
								currFlt = maybeLog(currShort)*fltFactor;
							}
							curr1 = (int)((currFlt - min1) * scale);
						}
						if( curr1 > max255)
							curr1 = max255;
						if( curr1 < 0) curr1 = 0;
			...
			default:
				...
				if( type1 == 2 || type1 == 5 || type1 == 6) buff1 = rawPixels.get(indx2);
				for( j=k=0; j<sizeY; j++) {
					j1 = j+pan1.y;
					badY = false;
					goodLungY = false;
		...
		if( type1 == 6) corSrc[0] = new MemoryImageSource(sizeX, sizeY, cm, buff, 0, sizeX);
	 ...
	 * type 5,6 is used in oblique with primary axial
	 * @param type 0 -> 3
	...
	double getZoom(int type) {
		...
		switch(type) {
			...
			case 5:	// oblique axial (source)
			case 6:	// oblique axial (destination)
				zoomTmp *= zoomX;
				break;
...
//		return zoomX * data1.y2xFactor * data1.y2XCnvtCor;
	...
	void UpdateAxialOblique( double axSlice, int secondaryType) {
		int i, x1, zg1, off1, off2, wid1 = data1.width, heigh1 = getNormalizedNumFrms();
		int slic1, type1=0, step1, iaxSlice, indx_1, indx0, num1;
		byte[] currByte = null, srcByt;
		short[] currShort = null, s_1, s0, s1, s2;
		...
		if(secondaryType == JFijiPipe.DSP_CORONAL) step1 = 1;
		else step1 = wid1;
		if( iaxSlice >= 0 && iaxSlice < heigh1) {
			switch( data1.depth) {
				case 32:
			...
			for( x1=num1=0; x1<wid1; x1++) {
				// here we assume that corFactor = sagFactor, i.e width=height
				sliceD = axSlice + (x1 - wid1/2.0)*obliqueFactor/corFactor;
				slic1 = ChoosePetCt.round(sliceD);
				if( slic1 < 0 || slic1 >= heigh1-1) continue;
				...
				switch( data1.depth) {
					...
					default:
						s_1 = data1.pixels.get(indx_1);
						scl_1 = data1.getRescaleSlope(indx_1)/data1.getMaxRescaleSlope();
						s0 = data1.pixels.get(indx0);
						scl0 = data1.getRescaleSlope(indx0)/data1.getMaxRescaleSlope();
						s1 = data1.pixels.get(++indx0);
						scl1 = data1.getRescaleSlope(indx0)/data1.getMaxRescaleSlope();
						if( indx0 < heigh1-1) indx0++;
						scl2 = data1.getRescaleSlope(indx0)/data1.getMaxRescaleSlope();
						s2 = data1.pixels.get(indx0);
//						srcPix = data1.pixels.get(zg1);
						for( i=0; i<wid1; i++) {
							off2 = off1+i*step1;
							if( off2 >= s0.length) continue;
							tmpShort = (short) (s_1[off2] + coef0);
							p4[0] = tmpShort * scl_1;
							tmpShort = (short) (s0[off2] + coef0);
							p4[1] = tmpShort * scl0;
							tmpShort = (short) (s1[off2] + coef0);
							p4[2] = tmpShort * scl1;
							tmpShort = (short) (s2[off2] + coef0);
							p4[3] = tmpShort * scl2;
							pres = bi3.getCubic(p4, y);
							currShort[off2] = limitShort(pres);
//							currShort[off1+i*step1] = limitShort(tmpShort, scale);
						}
				}
			}
			obliqueScale = sumRescale/num1;
			switch( data1.depth) {
				case 32:
	...
	void UpdateCoronalSagital( double corSlice, double sagSlice, int gateOffset, int type2) {
		...
		if( oldCorSlice != slic1 && corSlice >= 0 && slic1 < wid2 && slic1 >= 0) {
			...
			for( z1=0; z1<heigh1; z1++) {
				...
				off2 = z1*wid1;
				i1 = 0;
				zg1 = getCoronalDisplay(z1) + gateOffset;
	...
	int getCoronalDisplay(int z1) {
		if( CoronalDisplay == null) return z1;	// usual case
...
/*		int n = getNormalizedNumFrms();
	...
	short limitShort( short inVal, double scale) {
		return limitShort( inVal * scale);
	}

	short limitShort( double inVal) {
		double tmp = inVal;
	...
	protected class mriOff {
		int[] dbgVal = new int[9];
		int[] mriOffs = new int[9];	// 3*3=9
		int[] mriBegin = new int[6]; // initial alignment - 4 are never changed
		boolean isTable = false;	// use last 2 entries of mriBegin?
		private boolean isWider = false;
		double zm1 = 0;
//		static final int MINWIDTH1 = 30, MINWIDTH2 = 60;
//		private final int[] setSign = {1,1,-1};

/*		int debug() {
			return 0;	// a place for a break point
		}

		int getOff(int i) {	// not used
			return getOff(i, sliceType);	// current slice type
		}*/
		
		// This is used by ManualSync
		double getCleanOff(int i) {
			return getOffSub(i, sliceType, 2);
		}

		double getOff(int i, int j) {
			return getOffSub(i, j, 1);
		}

		private double getOffSub(int i, int j, int mode) {
			if( i == OFFZ0) return getBegin(j);
			if( i<OFFX || i>OFFZ || j<DSP_AXIAL || j>DSP_SAGITAL)
				return 0;
			double ret1 = mriOffs[i-OFFX+3*(j-DSP_AXIAL)];
			int tmpXY = data1.width;
			if( i>OFFY || mode==2) tmpXY = 1000;	// i.e. don't use
			if( tmpXY < 30) ret1 = ret1/10;
			else {
				if( tmpXY < 60) ret1 = ret1/5;
			}
//			if( useBegin && (getBegin(DSP_AXIAL)!=0 || getBegin(DSP_CORONAL)!=0)) {
			if( mode == 1) { // i.e. use getBegin
//				debugSub(i, j);
				switch(j) {
					case DSP_AXIAL:
						if( i==OFFX) ret1 += getBegin(DSP_AXIAL);
						if( i==OFFY) ret1 += getBegin(DSP_CORONAL);
						break;

					case DSP_CORONAL:
//						if( i==OFFX) ret1 -= getBegin(DSP_AXIAL);
						if( i==OFFZ) ret1 += 1.5*getBegin(DSP_CORONAL);
						break;

					case DSP_SAGITAL:
//						if( i==OFFX) ret1 -= 1.5*getBegin(DSP_CORONAL);
						if( i==OFFZ) ret1 -= 1.5*getBegin(DSP_AXIAL);
						break;
				}
			}
			return ret1;
...
/*		private int debugSub(int i, int j) {
			int i0, j0, idx;
			if( mriBegin[0]==0 && mriBegin[1]==0)
				return 0;	// this is probably the PET
			i0 = i-OFFX;
			j0 = j-DSP_AXIAL;
			idx = i0 + j0*3;
			if( mriBegin[i0] != 0 && dbgVal[idx] == 0)
				dbgVal[2] = 0;	// kill the data load attempts
			if( dbgVal[idx]++ > 3) return 0;
		...
		void copyOffs(JFijiPipe src) {
			...
			System.arraycopy(src.mri1.mriBegin, 0, mri1.mriBegin, 0, 6);
		}

		// Only ManualSync uses this write mechanism.
		void setOff(int i, int val) {
			setOff(i, val, sliceType);
		}

		private void setOff(int i, int val, int j) {
			if( i<OFFX || i>OFFZ || j<DSP_AXIAL || j>DSP_SAGITAL) return;
			mriOffs[i-OFFX+3*(j-DSP_AXIAL)] = val;
		}


		// this is used when the MRI is less than 1/3 the PET width
		...
		int getWid2() {
			int wid2 = data1.width;
			if( !isWider) wid2 /= 2;
			return wid2;
		}

		void setBegin(int i, int val) {
			int j;
			// OFFTBLX and Y are mutable and are set more than just once
			mriBegin[i-OFFX] = val;	// this is set only once, at start up
			if( i==OFFZ || i==OFFZ0)
				j = i;	
		}
...
//					p4[i+1][j+1] = j*10 + i;
	...
	protected class JData {
		...
		double maxVal, minVal, grandMax, sliceThickness, maxFactor, zratio;
...
//			if( spectFlg) IJ.log("Here is some spect data");
			...
			try {
				for( i=1; i<=numFrms; i++) {
					...
					if( trigTime != null) {
						tmp1 = "0018,1060";
						if(isDynamic) tmp1 = "0008,0032";
						Integer trigTime1 = ChoosePetCt.parseInt(ChoosePetCt.getDicomValue(meta, tmp1));
...
//							if( i == numFrms && posOff == 2) zEnd = zpos1; //axial
						}
					}
					zpos.add(zpos1);
...
//					localAcqTime = ChoosePetCt.getStudyDateTime(meta, 2);
				...
				tmp1 += "Then restart Fiji and try again.\n";
				tmp1 += "If this doesn't help ftp the study to ilan.tal@gmail.com";
				IJ.log(tmp1);
				return;
			}
			setMaxAndSort();
...
//					gb.setSigmaAndScaled(2.0, true);
		...
		void setStudyTime(int type, int time1) {
			Date changed = null;
			switch(type) {
				case 1:	// series time
					changed = serTime;
				...
				case 2:	// acquisition time
					changed = acquisitionTime;
					break;

				case 3:	// injection time
					changed = injectionTime;
					break;
			}
			if( changed == null) return;
			GregorianCalendar dat1 = new GregorianCalendar();
			dat1.setTime(changed);
			int day, month, year, hour, min, sec;
			day = dat1.get(GregorianCalendar.DAY_OF_MONTH);
			month = dat1.get(GregorianCalendar.MONTH);
			year = dat1.get(GregorianCalendar.YEAR);
			hour = time1/(60*60);
			min = (time1-(hour*60*60))/60;
			sec = time1 - hour*60*60 - min*60;
			dat1.set(year, month, day, hour, min, sec);
			changed.setTime(dat1.getTimeInMillis());
		}
		...
		boolean getMetaData(String meta, int forceSer) {
			...
			String tmp;
			metaData = meta;
			origSerType = seriesType = ChoosePetCt.getImageType(meta);
...
//			if( tmp != null) serTime = ChoosePetCt.getDateTime(tmp, null);
			...
			if( origSerType == ChoosePetCt.SERIES_SPECT || origSerType == ChoosePetCt.SERIES_SIEMENS_SPECT) {
				vals = new String[] {"F-10480","G-5191"};
				tmp = ChoosePetCt.findDicomValue(meta,"0008,0100", vals);
				if( tmp != null) isFeetFirst = true;
				double x = ChoosePetCt.parseDouble(ChoosePetCt.getDicomValue(meta, "0018,0088"));
				if( x<0) isFeetFirst = false;	// this counts from the top
			}
		...
		boolean isCt() {
			return (SOPclass == ChoosePetCt.SOPCLASS_TYPE_CT ||
					SOPclass == ChoosePetCt.SOPCLASS_TYPE_ENHANCED_CT);
		}
		...
		double maybeFixShift() {
			...
			if( Math.abs(pixelCenter[1]) < halfPixY/10) {
				pixelCenter[1] = 0;
				alreadyZero = true;
			}
			if(alreadyZero) return 0;
			// subtract off 1/2 pixel in both directions
			pixelCenter[0] -= halfPixX;
			pixelCenter[1] -= halfPixY;
		...
		int[] getSiemensSpect() {
			if(seriesType != ChoosePetCt.SERIES_SIEMENS_SPECT) return null;
			int[] retVals = new int[numFrms];
			for (int i=0; i<numFrms; i++) {
				retVals[i] = (int) SUVDialog.readSiemensPhilipsCrap(srcImage, 1, i);
			}
			return retVals;
		}
...
//					sagCut = ChoosePetCt.round((tmp1-1.0)*0.5*height);
		...
		protected void setMaxAndSort() {
			...
			if( numFrms>1) {
				...
				int[] siemensSpect = getSiemensSpect();
				if( siemensSpect != null && siemensSpect.length == numFrms) {
					isSiemens = true;
					IJ.log("Siemens SPECT. Please verify that it is correct.");
					siemensZval = zpos1[0];
					for(i=0; i<numFrms; i++) zpos1[i] = numFrms + 1 - siemensSpect[i];
				}
//				String tmp = ChoosePetCt.getDicomValue(metaData, "0019,9999");
				...
				if( dirty) {
					...
					for( i=0; i<numFrms; i++) {
						j0 = sortVect[i];
						zval = oldZpos.get(j0);
						if( isSiemens) zval = (float)((numFrms-1-i)*sliceThickness + siemensZval);
						zpos.add(zval);
			...
			for( i=0; i<numFrms; i++) {
				...
				if( pixels != null) {
					pix1 = pixels.get(i);
					n1 = pix1.length;
					if(pixRep == 0 && SOPclass != ChoosePetCt.SOPCLASS_TYPE_CT) {
						for(j1=0; j1<n1; j1++) {
							currVal1 = pix1[j1];
				...
				if( tmpd > grandMax) {
					grandMax = tmpd;
					if( pixFloat != null) maxPixel = (int) (grandMax / getRescaleSlope(i));
				}
				if( grandMax == 0 ) numBlankFrms = i+1;
			}
			if( pixFloat != null && maxPixel > 32767) {
				fltDataFactor = maxPixel / 32767.0;
				maxPixel = 32767;
			}
			mean = 0;
			maxVal = minVal = 0;
			...
			for( i=0; i<numFrms; i++) {
				if( i<numFrms-1) {
					zdif = zpos.get(i) - zpos.get(i+1);
					if( zdif != 0) {
						if( zmin == 0) zmin = zmax = zdif;
						else {
								if( zdif < zmin) zmin = zdif;
								if( zdif > zmax) zmax = zdif;
							}
					}
				}
			...
			if( SOPclass == ChoosePetCt.SOPCLASS_TYPE_CT || seriesType == ChoosePetCt.SER_FORCE_CT
				...
				if( pixFloat != null) rescaleIntercept = 0;
				if( pixRep == 0 && pixels != null) {
					// differentiate between Terry Roland and Azman
					pix1 = pixels.get(numFrms/2);
					if( pix1[0] < 0) rescaleIntercept = 0;
				}
...
/*			String tmp = "unknown";
		...
		// this is the depth of the stack compared to its axial width dimension
...
//			SUVfactor = 0.0;	// temporary - fix this!!
		...
		public lineEntry getLineOfData(int angle, int depth, int sliceNum) {
			...
			if( pixFloat != null) {
				...
				if( sliceNum >= pixels.size()) return ret1;
