...
//    public void writeImage(ImagePlus imp) throws IOException{
    public void writeAvi(String path, JFijiPipe mipPipe, SaveMip parent) throws IOException{
...
//        microSecPerFrame = (int)((1.0/Animator.getFrameRate())*1.0e6);
		microSecPerFrame = parent.getFrameTime() * 1000;
...
//       yDim = imp.getHeight();
 //      xDim = imp.getWidth();
	   zDim = mipPipe.data1.numFrms;
	   Dimension sz1 = parent.getWriteSize();
       ...
       writeInt(1); // dwScale
	   writeInt(1000 / parent.getFrameTime()); //  dwRate - frame rate for video streams
//       writeInt((int)Animator.getFrameRate()); //  dwRate - frame rate for video streams
    ...
    public void writeByteFrame(int slice, SaveMip parent) throws IOException {
/*        ImageProcessor ip = imp.getStack().getProcessor(slice);
        ip = ip.convertToByte(true);
        byte[] pixels = (byte[])ip.getPixels();
        int width = imp.getWidth();
        int height = imp.getHeight();*/
		byte[] pixels = parent.getPixels(slice);
		int width = xDim - xPad;
    ...
    public void createLUT(JFijiPipe mipPipe) {
        IndexColorModel cm = (IndexColorModel) mipPipe.cm;
        int mapSize = cm.getMapSize();
        lutWrite = new byte[4*256];
        for (int i = 0; i<256; i++) {
            if (i<mapSize) {
				lutWrite[4*i] = (byte)(255 - cm.getBlue(i));
				lutWrite[4*i+1] = (byte)(255 - cm.getGreen(i));
				lutWrite[4*i+2] = (byte)(255 - cm.getRed(i));
